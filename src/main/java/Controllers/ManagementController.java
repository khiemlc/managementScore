...
public class ManagementController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("btnLogin") != null) { //Nguoi dung da nhan nut submit
            ...
            AdministratorDAO adao = new AdministratorDAO();
            boolean admin1 = adao.checkEmailAdmin(email);
            if (admin1) {
                HttpSession session = request.getSession();
            ...
            TeacherDAO TDAO = new TeacherDAO();
            boolean admin1 = TDAO.checkMaileacher(email);
            java.util.Date currentDate = new java.util.Date();
            if (admin1) {
                HttpSession session = request.getSession();
            ...
            StudentDAO SDAO = new StudentDAO();
            boolean admin1 = SDAO.checkEmailStudent(email);
            java.util.Date currentDate = new java.util.Date();
            if (admin1) {
                HttpSession session = request.getSession();
            ...
            if (new_pass.equals(confirm_password)) {
                        ...
                        if (old_pass.equals(old_pass1)) {
                            if (!new_pass.equals(old_pass)) {
                                if (new_pass.equals(confirm_password)) {
                                    ...
                                    request.getSession().setAttribute("samepass111", "The new password must be the same as the confirm_password");
                ...
                request.getSession().setAttribute("samepass111", "The new password must be the same as the password confirmation");
